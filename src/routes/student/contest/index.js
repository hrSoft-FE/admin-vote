...
import { Card, Collapse, Icon, Row, Tag, Modal, Col, Tooltip, Table, Dropdown, Menu, Form } from 'antd'
import { color, newDate, urlEncode } from '../../../utils'
import { routerRedux } from 'dva/router'

const Panel = Collapse.Panel
const ContestPage = ({app, student, dispatch}) => {
  const {table, tableSignUp = [], tableSchoolAdmins = []} = student
  // 设置折叠面板的样式
  ...
  return (
    <div className='contest'>
      <Collapse defaultActiveKey={['sineUp', 'canRegister']}>
        <Panel header='可报名竞赛' key='canRegister' style={customPanelStyle}>
          {table.length === 0 && (<span>暂无可报名的比赛</span>)}
          <Row type='flex' gutter={12}>
            {table.map(item => {
              let extra
              if (item.signup) {
                extra = (<Tag color={color.green} onClick={() => SignUpContest(item)}>修改报姓名信息</Tag>)
              } else {
                if (item.can_register === 1) {
                  extra = (<Tag color={color.blue} onClick={() => SignUpContest(item)}>修改报姓名信息</Tag>)
              ...
              return (
                <Col xs={{span: 24}} sm={{span: 12}} xl={{span: 8}}
                  ...
                  <Card
                    ...
                    <div className='contest-item-contest'>
...
export default connect(({app, student, loading}) => ({
  app,
  student,
  loading
}))(Form.create()(ContestPage))