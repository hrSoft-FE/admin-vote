...
const SchoolInfoManage = ({app, schoolInfo, dispatch, form: {getFieldDecorator, validateFieldsAndScroll}}) => {
  ...
  const schoolInfoForm = (item, index) => {
    const {label, value, disabled = false} = item
    return (
      <div className='school-info-item' key={index}>
        <span className='school-info-label'>{label}</span>
        <span>：{user[value]}
          {!disabled &&
          <Icon onClick={() => dispatch(
            {type: 'schoolInfo/showModal', payload: value})}
                style={{fontSize: 16}} type='edit' />}
        </span>
      </div>
    )
  }
  return (
    <div className='school-info-wrapper'>
      {config.showConfig.map((item, index) => schoolInfoForm(item, index))}
      <Button
        type='primary' className='school-info-button'
        onClick={() => {
          dispatch({type: 'schoolInfo/showModal', payload: 'password'})
        }}>
        修改密码
      </Button>
      ...
      >
        <Form className='form-content'>
          {
            modal && config[modal].map(
              config => FormItemRender(config, getFieldDecorator,
                {initialValue: user[config.value]}))
          }
        </Form>
      </Modal>
    </div>
...
export default connect(({app, schoolInfo}) => ({app, schoolInfo}))(
  Form.create()(SchoolInfoManage))
